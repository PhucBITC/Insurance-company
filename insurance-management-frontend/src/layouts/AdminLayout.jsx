...
const AdminLayout = () => {
  ...
  const navItems = [
    ...
    { path: '/admin/assignments', name: 'Phân Công', icon: Users },
    { path: '/admin/logs', name: 'Lịch sử hệ thống', icon: Activity },
  ];
      ...
      <aside style={{
        ...
        <div style={{
          ...
          <div>
            ...
            <span style={{ fontSize: '0.75rem', color: 'var(--primary)', fontWeight: 'bold', tracking: '0.05em' }}>ADMIN PORTAL</span>
          </div>
        </div>
        ...
        <div style={{ padding: '20px 24px', borderBottom: '1px solid var(--glass-border)' }}>
          <div className="glass-card" style={{ padding: '16px', borderRadius: 'var(--radius-md)', background: 'rgba(255,255,255,0.02)' }}>
            <p style={{ fontSize: '0.75rem', color: 'var(--text-muted)' }}>Tài khoản đang đăng nhập</p>
            <p style={{ fontSize: '0.9rem', fontWeight: 'bold', textOverflow: 'ellipsis', overflow: 'hidden', whiteSpace: 'nowrap' }}>{user?.email}</p>
            <span className="badge badge-admin" style={{ marginTop: '8px' }}>ADMINISTRATOR</span>
          </div>
        </div>
            ...
            return (
              <Link
                ...
                style={{
                  ...
                  background: isActive ? 'linear-gradient(135deg, rgba(99, 102, 241, 0.1) 0%, rgba(168, 85, 247, 0.05) 100%)' : 'transparent',
